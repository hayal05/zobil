...
def _load_drive_credentials():
    try:
        ...
        logger.exception("Failed to load Google Drive service-account credentials.")
    return None
...
if _drive_credentials is None:
    logger.warning(
        "Google Drive integration is NOT configured (no GOOGLE_SERVICE_ACCOUNT_JSON or "
        "GOOGLE_SERVICE_ACCOUNT_FILE set). Orders can still be approved, but buyers will "
        "NOT automatically receive restricted access to their file until this is set up."
    )


def drive_service_account_email():
    """Returns the service account's email (to show the admin who to share
    Drive files with), or None if Drive integration isn't configured."""
    return getattr(_drive_credentials, "service_account_email", None)
...
async def _prompt_for_drive_file_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    sa_email = drive_service_account_email()
    if sa_email:
        setup_note = (
            f"⚠️ Make sure this file is shared in Google Drive with "
            f"<code>{esc(sa_email)}</code> as <b>Editor</b> — otherwise the bot can't "
            "grant buyers access to it."
        )
    else:
        setup_note = (
            "⚠️ Google Drive integration is not configured on the server yet "
            "(GOOGLE_SERVICE_ACCOUNT_JSON / GOOGLE_SERVICE_ACCOUNT_FILE is missing), "
            "so approvals won't be able to grant access automatically until that's set up."
        )
    await update.message.reply_text(
