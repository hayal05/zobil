...
async def browse_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Compact list: one single message with a button per product (title +
    # price), instead of a separate photo card for every item. Tapping a
    # product opens its full detail view (view_product) with photo,
    # description and the Buy Now button.
    lines = [header, ""]
    buttons = []
    for p in products:
        lines.append(f"• <b>{esc(p['title'])}</b> — {format_price(p['price'])}")
        buttons.append(
