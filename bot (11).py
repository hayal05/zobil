...
# Path to the welcome/banner image shown on /start. Defaults to the bundled
# assets/welcome.png next to this script; override with WELCOME_IMAGE_PATH
# if you want to swap it out without editing code.
WELCOME_IMAGE_PATH = os.environ.get(
    "WELCOME_IMAGE_PATH",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "welcome.png"),
)
...
def short_description(text, limit=120) -> str:
    """Trim a product description down to a short teaser for browsing cards."""
    if not text:
        return ""
    text = text.strip()
    if len(text) <= limit:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        "Browse and buy your favorite Books here\n\n"
        "To SELL your digital products contact us on @hayalsew5"
    )

    # Step 1: send the welcome image WITH the welcome text as its caption,
    # and no buttons yet, so image + text are the very first thing the
    # user sees.
    image_sent = False
    if WELCOME_IMAGE_PATH and os.path.isfile(WELCOME_IMAGE_PATH):
        try:
            with open(WELCOME_IMAGE_PATH, "rb") as photo:
                await update.message.reply_photo(photo, caption=text, parse_mode=ParseMode.HTML)
            image_sent = True
        except Exception:
            logger.exception("Failed to send welcome image from %s; falling back to text.", WELCOME_IMAGE_PATH)

    # Step 2: send the category buttons as a separate, second message.
    # If the image failed to send, fall back to putting the welcome text
    # on this message too, so it isn't lost.
    if image_sent:
        await update.message.reply_text("👇 Choose a category to get started:", reply_markup=categories_keyboard())
    else:
...
async def browse_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    header = f"{CATEGORY_EMOJI.get(category, '')} <b>{esc(category)}</b>"
    ...
    # The header is its own small message (edited in place if possible) since
    # each product below is shown as its own photo card and a photo message
    # can't be turned into a text-only one via edit_message_text.
    await safe_edit_or_send(query, f"{header}\n\n{len(products)} item(s) available:")

    for p in products:
        caption = (
            f"<b>{esc(p['title'])}</b>\n"
            f"💵 <b>Price:</b> {format_price(p['price'])}\n\n"
            f"{esc(short_description(p['description']))}"
        )
        ...
        if p["photo_file_id"]:
            await query.message.reply_photo(
                p["photo_file_id"], caption=caption, reply_markup=buy_button, parse_mode=ParseMode.HTML
            )
        else:
            await query.message.reply_text(caption, reply_markup=buy_button, parse_mode=ParseMode.HTML)

    await query.message.reply_text(
        "⬅️ Done browsing?", reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Back to Categories", callback_data="back_categories")]]
        )
    )
